...
foreach ($MODULE as $key => $value) {
  ...
  if (file_exists($file)) {
    echo '<link rel="stylesheet" href="'.PH_THEME_DIR.'/mod-'.$value.'/style.css'.SET_CACHE_HASH.'"/>'.PHP_EOL;
	}
}
?>
...
foreach ($MODULE as $key => $value) {
  ...
  if (file_exists($file)) {
      echo '<script src="'.PH_THEME_DIR.'/mod-'.$value.'/script.js'.SET_CACHE_HASH.'"></script>'.PHP_EOL;
  }
}
//Script 소스코드 설정 반영
if ($CONF['script']) {
